...
# 3. 답변 생성 프롬프트 (RAG)
qa_system_prompt = ( # 답변을 위한 시스템 지침 정의
    "당신은 병원 상담 챗봇입니다. 다음은 사람과 AI의 친근한 대화 내역입니다.\n"
    "이전 대화를 참고하여 사용자의 마지막 질문에 대해 참고 문서를 활용해 친절하고 정확하게 답변하세요.\n\n"
    "참고문서:\n{context}\n\n"
    "사용자 질문: {input}"
)

qa_prompt = ChatPromptTemplate.from_messages( # 답변 프롬프트 양식 정의
    [
        ("system", qa_system_prompt), # 시스템 지침
        ("placeholder", "{chat_history}"), # 대화 기록이 들어갈 자리
        ("human", "{input}"), # 사용자 질문
    ]
)
